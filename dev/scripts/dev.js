(($) => {
    ...
    (function ($) {
        $.fn.rewRellax = function () {
            ...
            const bodyWidth = DOMs.documentBody.width();

            const bodyCoof = bodyWidth / 1371.42;

            let make = function () {
                let rellaxDiv = $(this).attr('data-rellax-speed');
    ...
    $(window).on('load resize', function () {
        ...
        if (window.matchMedia("(min-width: 568px)").matches) {
            ...
            for (let i = 0; i < DOMs.gridMasonryItems.length; i++) {
    ...
    $(document).ready(() => {
        ...
        //  DESIGNER LINK
        const designerHover = function () {
            const desArr = DOMs.designerListLink;
            const desImgArr = DOMs.designerListPhoto;
